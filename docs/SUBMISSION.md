...
The time from a pollution signal to a cited, actionable recommendation is about a second when no spike is detected (median 1.1 s over the last 116 production runs); a few seconds when a spike triggers the enforcement worklist (median 3.4 s — the 95 s and 155 s outliers on 15 Aug predate the retrieval cache). That is measured in production, stamped per agent node, and shown in the console. It is not the time to an intervention: the officer's approval, the dispatch and the field visit are separate, timestamped steps in the same record, and the measured effect of each dispatch is tracked against the city's drift — so the whole chain, not just the model, is on the clock.
...
| Prediction interval coverage | Raw intervals under-covered at 48–63% against a nominal 80%; conformal calibration brought this to **0.783** on 207k Delhi test hours and **0.749** on Kolkata. We publish coverage **by predicted level**, not the marginal number alone: Kolkata holds ~0.80 in clean air and falls to **0.547 in the 56–76 µg/m³ band at +72 h**. Seven conformity scores were compared and none closed it — what fails is *conditional* coverage, which split conformal never promised. Delhi's worst band is 0.704, so it is one city's model, not the method. Every forecast also carries a calibrated P(>120) / P(>250) (Brier skill +51% / +31% vs climatology at 24 h) |
...
| Signal to cited recommendation | about 1 s of compute when no spike is detected, a few seconds when enforcement runs (median 3.4 s) — measured in production; approval → dispatch → closure timestamped per action |
