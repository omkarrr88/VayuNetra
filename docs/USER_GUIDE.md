...
| **Run locally** | `make dev` starts the API on :8000 and the web app on :5173. `make prewarm` wakes and verifies the live stack (GO / NO-GO). `make test` runs the 562 backend tests. |
...
| **test** | pytest -q --cov=. --cov-report=term-missing | Run all 562 tests with coverage report |
...
562 backend tests pass and the CI gate is 55% line coverage, but `api/main.py` — the largest single
...
If this manual says "562 tests", that command is how it was counted. Do not trust a number written in
