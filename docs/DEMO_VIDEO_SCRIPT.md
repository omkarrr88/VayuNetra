...
| Time | Screen / action | Say exactly |
|---|---|---|
| 1:00–1:15 | Press **7** (or click **Pipeline** in the top nav) → click **"Run agents live"** → the trace fills in node by node | "Five AI agents on one LangGraph run this city. Watch them live: orchestrator, attribution, forecast — then a spike gate decides whether the air needs enforcement at all — and advisory. Under a second end to end, and every node is traced and auditable." |
