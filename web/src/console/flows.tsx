...
export const FLOWS: Record<Section, Flow> = {
  action: {
    ...
    steps: [
      { n: 1, label: "Worst places now", info: <><P><b>What:</b> the map colours every ~1 km cell by its dominant pollution source; the status bar shows the worst cell and how fresh the data is.</P><P><b>Do:</b> click a hexagon to open its story — blame, forecast, act.</P></> },
  ...
  forecast: {
    ...
    steps: [
      ...
      { n: 3, label: "Who is in the forecast", info: <><P><b>What:</b> expected people in Very Poor / Severe air = Σ cell population × calibrated P(&gt; band); GPW population where sampled, cited city population otherwise. Exposure, not mortality.</P></> },
      { n: 4, label: "The past", info: <><P><b>What:</b> daily station means for 30 d / 90 d / 1 y with a plain-language verdict and spike-day markers, plus the last 48 h and the live source mix.</P></> },
    ],
  },
  citizen: {
    verb: "Inform",
